...
files at Maxim (as well as many other file types). This setup was designed to
work with version 8.0 of gvim/vim or higher.

Vim 8.0 is installed on all EOD linux machines here:
	/usr/sepp/bin/vim-8.0
	/usr/sepp/bin/gvim-8.0
	/usr/sepp/bin/vimdiff-8.0
	/usr/sepp/bin/gvimdiff-8.0

They can be accessed by adding the following to your ~/.my_aliases file:

	alias gvim "/usr/sepp/bin/gvim-8.0"
	alias vim "/usr/sepp/bin/vim-8.0"
	etc...
(close and reopen a terminal after doing this)

You may also want gvim-8.0 to be your default editor in virtuoso/p4. To do that, add these lines to your .my_cshrc file:

	setenv EDITOR /usr/sepp/bin/gvim-8.0
	setenv P4EDITOR /usr/sepp/bin/gvim-8.0
...
just in case you want to revert to your old setup later.
	...
	mv .vimrc ,vimrc_old
	mv .vim .vim_old
	unzip vim8.0_setup.zip

One modification needs to be made to the installed .vimrc file.
Open the file and change the fist line to be YOUR SSO username:

	let sso_name = 'first.last'
